...
export class MentionDirective implements OnInit, OnChanges {
  ...
  // search list container bounding client rect
  private searchListRect: ClientRect;
  ...
  measureText() {
    if (this.items.length) {
      const longest = this.items.reduce(function (a1, b1) { return a1.length > b1.length ? a1 : b1; });
      const text = longest.label;
      ...
      this.searchListRect = ul.getBoundingClientRect();
  ...
  showSearchList(nativeElement: HTMLInputElement) {
    if (this.searchList == null) {
      ...
      this.searchList.position(nativeElement, this.iframe, this.searchListRect);
